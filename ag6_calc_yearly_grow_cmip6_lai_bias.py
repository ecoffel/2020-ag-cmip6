...
yearly_lai_grow_bias = np.full([len(range(1982, 2014+1)), \
                                   era5_lai_grow_regrid.lat.values.shape[0], \
                                   era5_lai_grow_regrid.lon.values.shape[0]], np.nan)


print('processing %s...'%model)
for y, year in enumerate(range(1982, 2014+1)):
    print('year %d...'%year)
    for xlat in range(yearly_lai_grow_bias.shape[1]):
        for ylon in range(yearly_lai_grow_bias.shape[2]):
