...
print('loading regridded ef for %s'%model)
cmip6_ef_grow = xr.open_dataset('cmip6_output/growing_season/cmip6_%s_grow_ef_mon_%s_%s_regrid.nc'%(crop, region, model))
...
yearly_ef_grow_bias = np.full([len(range(1981, 2014+1)), \
...
for y, year in enumerate(range(1981, 2014+1)):
    print('year %d...'%year)
    for xlat in range(yearly_ef_grow_bias.shape[1]):
        for ylon in range(yearly_ef_grow_bias.shape[2]):
            yearly_ef_grow_bias[y, xlat, ylon] = cmip6_ef_grow.grow_ef.values[y, xlat, ylon] - \
                                                        era5_ef_grow_regrid.ef_grow.values[y, xlat, ylon]

with open('cmip6_output/bias/yearly-cmip6-era5-ef-grow-bias-%s-%s.dat'%(region, model), 'wb') as f:
    pickle.dump(yearly_ef_grow_bias, f)