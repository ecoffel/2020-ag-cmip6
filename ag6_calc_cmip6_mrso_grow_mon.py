...
cmip6_mrso_hist = xr.open_mfdataset('%s/%s/r1i1p1f1/historical/mrso/mrso_Lmon_*.nc'%(dirCmip6, model), concat_dim='time')

print('selecting data for %s...'%model)
cmip6_mrso_hist = cmip6_mrso_hist.sel(time=in_time_range(cmip6_mrso_hist['time.year']))

cmip6_mrso_hist.load()


# regrid sacks data to current model res
regridMesh_cur_model = xr.Dataset({'lat': (['lat'], cmip6_mrso_hist.lat),
                                   'lon': (['lon'], cmip6_mrso_hist.lon)})
...
yearly_groups = cmip6_mrso_hist.groupby('time.year').groups
yearly_grow_mrso = np.full([2014-1981+1, cmip6_mrso_hist.lat.size, cmip6_mrso_hist.lon.size], np.nan)

# count up all non-nan grid cells so we can estimate percent complete
ngrid = 0
for xlat in range(cmip6_mrso_hist.lat.size):
    for ylon in range(cmip6_mrso_hist.lon.size):
        ...
        if ~np.isnan(sacksStart_regrid[xlat, ylon]) and ~np.isnan(sacksEnd_regrid[xlat, ylon]):
            ...
            if sacksStart_regrid[xlat, ylon] > sacksEnd_regrid[xlat, ylon]:
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))[1:]):
                    ...
                    cur_mrso1 = cmip6_mrso_hist['mrso'][np.array(yearly_groups[year-1])[int(sacksEnd_regrid[xlat, ylon]):], xlat, ylon]
                    cur_mrso2 = cmip6_mrso_hist['mrso'][np.array(yearly_groups[year])[:int(sacksStart_regrid[xlat, ylon])], xlat, ylon]

                    cur_mrso = np.nanmean(np.concatenate([cur_mrso1, cur_mrso2]))

                    if not np.isnan(cur_mrso):
                        yearly_grow_mrso[y, xlat, ylon] = cur_mrso
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))):
                    ...
                    cur_mrso = np.nanmean(cmip6_mrso_hist['mrso'][np.array(yearly_groups[year])[int(sacksStart_regrid[xlat, ylon]):int(sacksEnd_regrid[xlat, ylon])], xlat, ylon])
                    
                    if not np.isnan(cur_mrso):
                        yearly_grow_mrso[y, xlat, ylon] = cur_mrso
                n += 1

da_grow_mrso = xr.DataArray(data   = yearly_grow_mrso, 
                      dims   = ['time', 'lat', 'lon'],
                      coords = {'time': list(yearly_groups.keys()), 'lat':cmip6_mrso_hist.lat, 'lon':cmip6_mrso_hist.lon},
                      attrs  = {'units'     : 'Fraction'
                        })
ds_grow_mrso = xr.Dataset()
ds_grow_mrso['grow_mrso'] = da_grow_mrso

print('saving netcdf...')
ds_grow_mrso.to_netcdf('cmip6_output/growing_season/cmip6_%s_grow_mrso_mon_%s_%s.nc'%(crop, region, model))
