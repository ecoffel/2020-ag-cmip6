...
cmip6_cropFrac_hist = xr.open_mfdataset('%s/%s/r1i1p1f1/historical/cropFrac/cropFrac_Lmon_*.nc'%(dirCmip6, model), concat_dim='time')

print('selecting data for %s...'%model)
cmip6_cropFrac_hist = cmip6_cropFrac_hist.sel(time=in_time_range(cmip6_cropFrac_hist['time.year']))

cmip6_cropFrac_hist.load()

# regrid sacks data to current model res
regridMesh_cur_model = xr.Dataset({'lat': (['lat'], cmip6_cropFrac_hist.lat),
                                   'lon': (['lon'], cmip6_cropFrac_hist.lon)})
...
yearly_groups = cmip6_cropFrac_hist.groupby('time.year').groups
yearly_grow_cropFrac = np.full([2014-1981+1, cmip6_cropFrac_hist.lat.size, cmip6_cropFrac_hist.lon.size], np.nan)

# count up all non-nan grid cells so we can estimate percent complete
ngrid = 0
for xlat in range(cmip6_cropFrac_hist.lat.size):
    for ylon in range(cmip6_cropFrac_hist.lon.size):
        ...
        if ~np.isnan(sacksStart_regrid[xlat, ylon]) and ~np.isnan(sacksEnd_regrid[xlat, ylon]):
            ...
            if sacksStart_regrid[xlat, ylon] > sacksEnd_regrid[xlat, ylon]:
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))[1:]):
                    ...
                    cur_cropFrac1 = cmip6_cropFrac_hist['cropFrac'][np.array(yearly_groups[year-1])[int(sacksStart_regrid[xlat, ylon]):], xlat, ylon]
                    cur_cropFrac2 = cmip6_cropFrac_hist['cropFrac'][np.array(yearly_groups[year])[:int(sacksEnd_regrid[xlat, ylon])], xlat, ylon]

                    cur_cropFrac = np.nanmean(np.concatenate([cur_cropFrac1, cur_cropFrac2]))

                    if not np.isnan(cur_cropFrac):
                        yearly_grow_cropFrac[y, xlat, ylon] = cur_cropFrac
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))):
                    ...
                    cur_cropFrac = np.nanmean(cmip6_cropFrac_hist['cropFrac'][np.array(yearly_groups[year])[int(sacksStart_regrid[xlat, ylon]):int(sacksEnd_regrid[xlat, ylon])], xlat, ylon])
                    
                    if not np.isnan(cur_cropFrac):
                        yearly_grow_cropFrac[y, xlat, ylon] = cur_cropFrac
                n += 1

da_grow_cropFrac = xr.DataArray(data   = yearly_grow_cropFrac, 
                      dims   = ['time', 'lat', 'lon'],
                      coords = {'time': list(yearly_groups.keys()), 'lat':cmip6_cropFrac_hist.lat, 'lon':cmip6_cropFrac_hist.lon},
                      attrs  = {'units'     : 'Fraction'
                        })
ds_grow_cropFrac = xr.Dataset()
ds_grow_cropFrac['grow_cropFrac'] = da_grow_cropFrac

print('saving netcdf...')
ds_grow_cropFrac.to_netcdf('cmip6_output/growing_season/cmip6_%s_grow_cropFrac_mon_%s_%s_fixed_sh.nc'%(crop, region, model))
