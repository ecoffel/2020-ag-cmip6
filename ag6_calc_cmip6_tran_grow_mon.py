...
cmip6_tran_hist = xr.open_mfdataset('%s/%s/r1i1p1f1/historical/tran/tran_Lmon_*.nc'%(dirCmip6, model), concat_dim='time')
...
regridMesh_cur_model = xr.Dataset({'lat': (['lat'], cmip6_tran_hist.lat),
                                   'lon': (['lon'], cmip6_tran_hist.lon)})
...
ngrid = 0
for xlat in range(cmip6_tran_hist.lat.size):
    for ylon in range(cmip6_tran_hist.lon.size):
        ...
        if ~np.isnan(sacksStart_regrid[xlat, ylon]) and ~np.isnan(sacksEnd_regrid[xlat, ylon]):
            ...
            if sacksStart_regrid[xlat, ylon] > sacksEnd_regrid[xlat, ylon]:
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))[1:]):
                    ...
                    if not np.isnan(cur_tran):
                        yearly_grow_tran[y, xlat, ylon] = cur_tran*60*60*24
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))):
                    ...
                    cur_tran = np.nanmean(cmip6_tran_hist['tran'][np.array(yearly_groups[year])[int(sacksStart_regrid[xlat, ylon]):int(sacksEnd_regrid[xlat, ylon])], xlat, ylon])
                    
                    if not np.isnan(cur_tran):
                        yearly_grow_tran[y, xlat, ylon] = cur_tran*60*60*24
                n += 1

da_grow_tran = xr.DataArray(data   = yearly_grow_tran, 
                      dims   = ['time', 'lat', 'lon'],
                      coords = {'time': list(yearly_groups.keys()), 'lat':cmip6_tran_hist.lat, 'lon':cmip6_tran_hist.lon},
                      attrs  = {'units'     : 'mm'
                        })
ds_grow_tran = xr.Dataset()
