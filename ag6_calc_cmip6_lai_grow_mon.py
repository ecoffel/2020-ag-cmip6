...
cmip6_lai_hist = xr.open_mfdataset('%s/%s/r1i1p1f1/historical/%s/%s_Lmon_*.nc'%(dirCmip6, model, var, var), concat_dim='time')


print('selecting data for %s...'%model)
cmip6_lai_hist = cmip6_lai_hist.sel(time=in_time_range(cmip6_lai_hist['time.year']))

cmip6_lai_hist.load()

# regrid sacks data to current model res
regridMesh_cur_model = xr.Dataset({'lat': (['lat'], cmip6_lai_hist.lat),
                                   'lon': (['lon'], cmip6_lai_hist.lon)})
...
yearly_groups = cmip6_lai_hist.groupby('time.year').groups
yearly_grow_lai = np.full([2014-1981+1, cmip6_lai_hist.lat.size, cmip6_lai_hist.lon.size], np.nan)

# count up all non-nan grid cells so we can estimate percent complete
ngrid = 0
for xlat in range(cmip6_lai_hist.lat.size):
    for ylon in range(cmip6_lai_hist.lon.size):
        ...
        if ~np.isnan(sacksStart_regrid[xlat, ylon]) and ~np.isnan(sacksEnd_regrid[xlat, ylon]):
            ...
            if sacksStart_regrid[xlat, ylon] > sacksEnd_regrid[xlat, ylon]:
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))[1:]):
                    ...
                    cur_lai1 = cmip6_lai_hist[var][np.array(yearly_groups[year-1])[int(sacksStart_regrid[xlat, ylon]):], xlat, ylon]
                    cur_lai2 = cmip6_lai_hist[var][np.array(yearly_groups[year])[:int(sacksEnd_regrid[xlat, ylon])], xlat, ylon]

                    cur_lai = np.nanmean(np.concatenate([cur_lai1, cur_lai2]))

                    if not np.isnan(cur_lai):
                        yearly_grow_lai[y, xlat, ylon] = cur_lai
                ...
                for y,year in enumerate(np.array(list(yearly_groups.keys()))):
                    ...
                    cur_lai = np.nanmean(cmip6_lai_hist[var][np.array(yearly_groups[year])[int(sacksStart_regrid[xlat, ylon]):int(sacksEnd_regrid[xlat, ylon])], xlat, ylon])
                    
                    if not np.isnan(cur_lai):
                        yearly_grow_lai[y, xlat, ylon] = cur_lai
                n += 1

da_grow_lai = xr.DataArray(data   = yearly_grow_lai, 
                      dims   = ['time', 'lat', 'lon'],
                      coords = {'time': list(yearly_groups.keys()), 'lat':cmip6_lai_hist.lat, 'lon':cmip6_lai_hist.lon},
                      attrs  = {'units'     : 'LAI'
                        })
ds_grow_lai = xr.Dataset()
ds_grow_lai['grow_lai'] = da_grow_lai

print('saving netcdf...')
ds_grow_lai.to_netcdf('cmip6_output/growing_season/cmip6_%s_grow_%s_mon_%s_%s_fixed_sh.nc'%(var, crop, region, model))
